...
tokens, iterations, prompt sizes) lives in the JSONL log and the step-13 log
viewer, which own "where did my message go" debugging. This front-end never
shows it.
...
class Tui(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="inputrow"):
            ...
            yield Input(placeholder="Type a message…  (Ctrl+T: tabs · Ctrl+F: "
                        "search · Ctrl+P: commands)", id="input")
    ...
    def on_log_event(self, message: LogEvent) -> None:
        ...
        if phase == "tool_call" and new_cards:
    ...
    def on_repl_notice(self, message: ReplNotice) -> None:
        # With the feed silenced, on_output carries the agent's final reply
        # (the summary or a question, shown as the current thinking/objective)
        # and the occasional error notice, which is surfaced as an error card.
        text = message.text
    ...
    def _render_header(self) -> str:
        ...
        servers = self._repl.servers
        status = (" ".join(f"{n}({c})" for n, c in servers.items()) + " ok"
                  if servers and len(self._repl.registry) else "NO TOOLS")
        parts = [f" {status}"]
        if vit["hp"] is not None:
            parts.append(f"HP {vit['hp']}/{vit['max_hp']}" if vit["max_hp"]
                         else f"HP {vit['hp']}")
        if vit["mana"] is not None:
            parts.append(f"Mana {vit['mana']}")
        if vit["moves"] is not None:
            parts.append(f"Moves {vit['moves']}")
        if char["level"] is not None:
            parts.append(f"Lv {char['level']}")
        if char["gold"] is not None:
            parts.append(f"Gold {char['gold']}")
        status = self._journey.state.status
        if status["stance"]:
            parts.append(status["stance"])
        if status["hungry"]:
            parts.append("hungry")
        if status["thirsty"]:
            parts.append("thirsty")
        parts.append(self._ctx_label().strip(" ·"))
        parts.append(f"${round(self._repl.cost, 4)}")
        parts.append(time.strftime("%H:%M:%S"))
        return " · ".join(p for p in parts if p)

    def _ctx_label(self) -> str:
        """CURRENT window occupancy (latest call's input), not lifetime spend."""
    ...
    def _progress_text(self) -> str:
        """Liveness, not a trace: a spinner and the current humanized action."""
        if self._live["active"]:
            ...
            return f"{frame} playing: {action} · {secs}s"
        return f"  [ready] · {self._repl.turn} turns"
