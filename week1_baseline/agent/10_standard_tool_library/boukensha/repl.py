...
from .errors import ApiError, ConfigError, LoopError
...
class Repl:
    ...
    def start(self) -> None:
        ...
        while True:
            ...
            # A line beginning with // is an in-character line that starts with
            # a slash: drop one slash and send the rest to the agent.
            if line.startswith("//"):
                self.run_turn(line[1:])
                continue
    ...
    def _on_reasoning(self, event: dict) -> None:
        # Inert until context management (step 12) emits reasoning events.
    ...
    def _cmd_compact(self, rest: str) -> bool:
        compact = getattr(self._context, "compact_messages", None)
        if compact is None:
            self._writeln("(compaction is available from the context step)")
            return True
        dropped = compact()
        self._writeln(f"compacted context — dropped {dropped} message(s)")
        return True
