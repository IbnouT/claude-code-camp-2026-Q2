...
ROOTS = [
    ...
    *sorted((Path(__file__).resolve().parents[2] / "agent").glob("*/tests")),
]
...
class TestNoTestHidesBelowTheMainGuard(unittest.TestCase):
    def test_there_are_files_to_check(self):
        ...
        # failure mode of every check in this file's own category.
        files = list(_files())
        # Every step from 00 to 13 plus this package. A floor rather than the
        # exact number, so a new test file does not fail this while an empty
        # or mis-rooted scan still does.
        self.assertGreater(len(files), 35, f"only found {len(files)} test files")
