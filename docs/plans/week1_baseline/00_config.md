...
run, and the found directory's `.env` is loaded — you trust the tree you run
in.
...
- More characters later are additive: a list of identities under `mud:`, each
  password resolved as `MUD_PASSWORD_<USERNAME>` from `.env` — nothing in this
  schema changes.
...
The example prints the resolved configuration — directory, tasks,
provider/model, resolved system prompt, MUD target, and whether the API key is
set — and asserts:
...
both assertions pass, a missing `settings.yaml`/`.env` still runs on defaults,
