...
**Observability**
- **Log viewer**: a browser view of a session, reading the logger's JSONL. A separate
  program rather than part of the agent, coupled to it only through the file format, so
  it can read a log written by any version.
...
| 13 | context economics | prompt caching, per-class cost, and honest token metrics |

The table is the agent's assembly, so the log viewer is not a row in it. It is a
separate program at `week1_baseline/log_viewer/`, unnumbered because it is not a step,
reading the session logs the agent writes and importing nothing from it.
...
- The standard library is preferred, and dependencies are kept minimal.
