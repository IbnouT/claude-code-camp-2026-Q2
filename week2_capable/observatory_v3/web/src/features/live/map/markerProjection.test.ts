...
import type { WorldEdge, WorldFrontier, WorldNode } from "./world-types"
...
describe("map evidence marker projection", () => {
  ...
  it("starts every planar frontier at the correct room edge", () => {
    expect(frontierGeometry({ x: 100, y: 200 }, "north").start).toEqual({
      x: 132,
      y: 200,
    })
    expect(frontierGeometry({ x: 100, y: 200 }, "east").start).toEqual({
      x: 164,
      y: 232,
    })
    expect(frontierGeometry({ x: 100, y: 200 }, "southwest").start).toEqual({
      x: 100,
      y: 264,
    })
  })
  ...
  it("canonicalizes duplicate rooms and de-duplicates source directions", () => {
    ...
    const projection = projectMapEvidence(
      [first, repeated],
      [],
      [frontier("place:1", "n", [10]), frontier("place:2", "north", [12])],
      rooms
    )
  ...
  it("exposes only marker kinds produced by current evidence", () => {
    ...
    expect([...projection.markerKinds].sort()).toEqual(["frontier", "visits"])
  })
})

function room(id: string, vnum: number, exits: string[]): WorldNode {
  return {
    id,
    place: vnum,
    title: `Room ${vnum}`,
...
function edge(source: string, target: string, direction: string): WorldEdge {
  return {
