...
export function projectRoomInspector(
  ...
  for (const item of frontier) {
    ...
    frontierByDirection.set(
      direction,
      mergeNumbers(frontierByDirection.get(direction) ?? [], item.evidence)
    )
  }
  const confirmed = new Set(selected.exits.map(normalizeDirection))
  const exits = [
  ...
  const exitEvidence = new Set([...frontierByDirection.values()].flat())

  return {
    id: selectedId,
    title: selected.title,
    atlas:
      selected.atlas === null || selected.atlas === undefined
        ? null
        : {
