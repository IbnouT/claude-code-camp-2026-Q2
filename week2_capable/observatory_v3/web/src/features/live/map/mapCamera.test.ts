...
describe("map camera geometry", () => {
  ...
  it("starts a drag from the exact camera without recentering or zooming", () => {
    ...
    expect(panMapCamera(view, { x: 4, y: -6 }, { x: 0.8, y: 0.8 })).toEqual({
  ...
  it("keeps settled Follow targets independent of animation progress", () => {
    ...
    for (const target of rooms) {
      steppedAnchor = resolveFollowMapCameraAnchor(steppedAnchor, target, frame)
  ...
  it("critically damps camera motion without crossing its target", () => {
    ...
    for (let frame = 0; frame < 120; frame += 1) {
      motion = stepCriticallyDampedMapCenter(motion, { x: 80, y: 20 }, 1 / 60)
...
function room(id: string, state: WorldNode["state"]): WorldNode {
  return {
    id,
    place: 1,
    title: id,
