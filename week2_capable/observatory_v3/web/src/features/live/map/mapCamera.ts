...
export function zoomMapCamera(
  ...
  const nextScale = direction === "in" ? view.scale * 1.25 : view.scale / 1.25
  return {
    center: view.center,
...
export function resolveFollowMapCameraAnchor(
  ...
  const followed = followMapCameraWithinDeadZone(anchor, target, frame)
  return focus === null
    ? followed
    : clampFocusCamera(followed, focus.agent, focus.extent, frame)
}
...
export function clampFocusCamera(
  ...
  const maximumXOffset = Math.max(
    0,
    Math.min(viewport.width / 4, rightDistance + mapRoomWidth)
  )
  const minimumXOffset = -Math.max(
    0,
    Math.min(viewport.width / 4, leftDistance + mapRoomWidth)
  )
  const maximumYOffset = Math.max(
    0,
    Math.min(viewport.height / 4, bottomDistance + mapRoomHeight)
  )
  const minimumYOffset = -Math.max(
    0,
    Math.min(viewport.height / 4, topDistance + mapRoomHeight)
  )
  return {
    center: {
      x:
        agent.x +
        clamp(view.center.x - agent.x, minimumXOffset, maximumXOffset),
      y:
        agent.y +
        clamp(view.center.y - agent.y, minimumYOffset, maximumYOffset),
    },
    scale: view.scale,
  }
}
...
export function resolveMapViewport({
  ...
  const currentCenter =
    roomCenter(graph, graph.currentRoomId) ?? viewportCenter(activeExtent)
  ...
  if (camera === "fit" || (camera === "follow" && fitOnFollow)) {
    ...
    const center =
      camera === "manual" ? (manualCenter ?? currentCenter) : currentCenter
    const width =
      Math.min(
    ...
    viewport =
      camera === "manual"
        ? centerMapViewportInExtent(completeExtent, { width, height }, center)
        : {
            x: center.x - width / 2,
            y: center.y - height / 2,
            width,
  ...
  return {
    viewport,
    panning:
      completeExtent.width > viewport.width ||
...
export function keepSelectedRoomOutsidePanel(
  ...
  if (roomPoint === null || frame.width <= 0 || frame.height <= 0) {
    return viewport
  }
  const horizontalScale = viewport.width / frame.width
  const verticalScale = viewport.height / frame.height
