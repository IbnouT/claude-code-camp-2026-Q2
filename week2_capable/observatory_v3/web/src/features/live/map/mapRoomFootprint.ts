...
export function mapRoomFootprint(
  ...
  const titleLeft = point.x + (mapRoomWidth - titleWidth) / 2
  ...
  const right = Math.max(
    ...
    titleRight,
    node.visits > 1 ? point.x + mapRoomVisitBadgeRight : point.x + mapRoomWidth
  )
  const top = Math.min(
    point.y,
...
export function truncateMapRoomTitle(title: string): string {
  let displayed =
    title.length > mapRoomTitleCharacterLimit
