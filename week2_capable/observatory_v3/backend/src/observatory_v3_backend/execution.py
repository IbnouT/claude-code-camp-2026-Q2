...
class ExperimentExecutor:
    ...
    async def start(self, job_id: str) -> ExperimentJob:
        ...
        job.state = "running"
        await self.storage.run(self._persist, job)
    ...
    async def _run_sample(
        ...
        await self.storage.run(output.mkdir, parents=True, exist_ok=True)
        ...
        sample["detail"] = str(
            record.get("error") or record.get("stop_reason") or sample["state"]
        )
        await self.storage.run(self._persist, job)
    ...
    def _load(self) -> None:
        ...
        for path in self.state_root.glob("jobs/*/job.json"):
            try:
                ...
                samples = {sample["id"]: sample for sample in row.get("samples", [])}
                job = ExperimentJob(
                    id=str(row["id"]),
