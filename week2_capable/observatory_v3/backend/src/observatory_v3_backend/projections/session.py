...
DIAGNOSTIC_COVERAGE: tuple[DiagnosticKind, ...] = (
    "false_completion",
    "belief_divergence",
    "position_ambiguity",
    "confusion_loop",
...
def project_recorded_session(
    ...
    gateway_records, gateway_terminal = _gateway_records(
        bundle.gateway_rows,
        trace_links,
        {record.id: (record.iteration, record.turn) for record in agent_records},
    )
    ...
    gateway_session = bundle.gateway_rows[0].session if bundle.gateway_rows else None
...
def project_recorded_session_prefix(
    ...
    gateway_sequence = max(
        (record.sequence for record in records if record.source == "gateway"),
        default=0,
    )
    world = project_world(
    ...
    points = tuple(
        point for point in investigation.cost.points if point.record_id in retained
    )
    diagnostics = tuple(
        item
...
def _record_order(
    ...
    try:
        stamp = datetime.fromisoformat(record.at.replace("Z", "+00:00")).timestamp()
    except ValueError:
...
def _prefix_cost(
    ...
    return ledger.model_copy(
        update={
            ...
            "fresh_input_tokens": sum(point.fresh_input_tokens for point in points),
            "cache_read_tokens": sum(point.cache_read_tokens for point in points),
            "cache_write_tokens": sum(point.cache_write_tokens for point in points),
...
def _objective(rows: tuple[dict[str, Any], ...]) -> str | None:
    ...
    for row in rows:
        task = row.get("task")
        if (
            row.get("phase") in {"config", "session_start"}
            and isinstance(
                task,
                str,
            )
    ...
    for line, row in enumerate(rows, start=1):
        ...
        records.append(
            SessionEvidenceRecord(
                ...
                capture_gaps=(
                    ("No timestamp was retained",) if not row.get("at") else ()
                ),
            )
        )
...
def _outcome_record(
    ...
    status: EvidenceStatus = "complete" if summary.success else "failed"
...
def _cost_ledger(
    ...
    for index, (record, row) in enumerate(response_rows):
        ...
        if curve is not None:
            ...
            pricing_source: PricingSource = "attempt_cost_curve"
        else:
            attributed_cost = raw_cost
            pricing_source = "agent_response"
        next_row = (
            response_rows[index + 1][1] if index + 1 < len(response_rows) else None
        )
        points.append(
            SessionCostPoint(
                record_id=record.id,
    ...
    raw_response_total = sum(point.raw_response_cost_usd for point in points)
    ...
    return SessionCostLedger(
        ...
        fresh_input_tokens=int(bundle.record.get("fresh_input_tokens", 0) or 0),
        cache_read_tokens=int(bundle.record.get("cache_read_tokens", 0) or 0),
        cache_write_tokens=int(bundle.record.get("cache_write_tokens", 0) or 0),
...
def _capture_gaps(
    ...
    gaps: list[str] = []
    agent_phases = {str(row.get("phase") or "") for row in bundle.agent_rows}
    ...
    if transformed_results and any(
        not isinstance(row.get("stages"), dict) for row in transformed_results
    ):
        gaps.append("Tool result transformation stages are missing")
    if gateway_kinds & {"wire", "observation", "unparsed"} and not {
        "wire_text",
        "parser_input",
    }.issubset(gateway_kinds):
        ...
        if final_belief is not None:
            findings.append(
                _diagnostic(
                    ...
                    alternatives=("The final account may describe partial progress",),
        ...
        if position_records:
            findings.append(
                _diagnostic(
                    ...
                    consequence=("Spatial conclusions cannot assume one unique room."),
    ...
    if repeated and repeated[0][0] and repeated[0][1] >= 5:
        ...
        findings.append(
            _diagnostic(
                ...
                alternatives=("The repeated command may be required by a long route",),
    ...
    if parse_misses > 0:
        ...
        findings.append(
            _diagnostic(
                ...
                alternatives=("The residual may be decorative output",),
    ...
    if corrections + invalid >= 3:
        ...
        findings.append(
            _diagnostic(
                ...
                alternatives=("One external failure may have caused several retries",),
    ...
    if occupancy > 0 and fresh > occupancy * 3 and summary.iterations >= 10:
        ...
        findings.append(
            _diagnostic(
                ...
                alternatives=("Long context may still contain necessary constraints",),
                affected=("Context efficiency", "Run cost"),
            )
        )
    if capture_gaps:
        gap_records = tuple(record.id for record in records if record.capture_gaps)
        findings.append(
            _diagnostic(
    ...
    for record in records:
        ...
        if (
            ...
            return _diagnostic(
                ...
                at_record=record.id,
                evidence=(record.id,),
                alternatives=("The world state may be stable over this interval",),
...
def _lens(
    ...
    truth = next(
        (record for record in records if record.id == "benchmark:outcome"),
        None,
    )
    return EvidenceLens(
...
def _agent_form(phase: str) -> EvidenceFormKind:
...
def _gateway_form(kind: str) -> EvidenceFormKind:
    if kind == "wire":
        return "wire"
...
def _agent_status(row: dict[str, Any]) -> EvidenceStatus:
...
def _gateway_status(row: GatewayEvidenceRow) -> EvidenceStatus:
...
def _gateway_label(row: GatewayEvidenceRow) -> str:
    ...
    if row.kind in {"tool_call", "tool_result", "tool_rejected"}:
        ...
        return f"{label}: {row.payload.get('tool', 'unknown')}"
...
def _first_string(
    ...
    return next(
        (str(row[key]) for row in rows if isinstance(row.get(key), str) and row[key]),
        None,
    )
