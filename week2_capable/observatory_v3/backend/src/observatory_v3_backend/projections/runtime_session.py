...
def project_runtime_session(
    ...
    *,
    ...
    capture_gaps = tuple(
        dict.fromkeys(
            (
                *snapshot.capture_gaps,
                *_evidence_capture_gaps(agent_events, gateway_events),
            )
        )
    )
    diagnostics = _instrumentation_diagnostics(capture_gaps, records)
    iterations = sum(event.get("phase") == "iteration" for event in agent_events)
...
def _agent_records(
    ...
    for event in events:
        ...
        if phase == "session_start":
            ...
            parent = (
                (tool_calls.get(tool_id) if isinstance(tool_id, str) else None)
                or response
                or iteration
                or turn
                or root
        ...
        cost = _number(event.get("cost_usd")) if phase == "response" else 0
...
def _operator_records(
    ...
    boundaries = [record for record in agent_records if record.kind == "iteration"]
...
def _cost_points(
    ...
    for event in events:
        ...
        fresh = _integer(
            normalized.get("input_tokens") or normalized.get("prompt_tokens")
        )
        cache_read = _integer(
            normalized.get("cache_read_input_tokens") or normalized.get("cached_tokens")
        )
        ...
        output = _integer(
            normalized.get("output_tokens") or normalized.get("completion_tokens")
        )
...
def _instrumentation_diagnostics(
    ...
    return (
        SessionDiagnostic(
            ...
            consequence=(
                "The available run can be followed, but some conclusions "
                "remain bounded."
            ),
...
def _evidence_capture_gaps(
    ...
    if transformed_results and any(
        not isinstance(event.get("stages"), dict) for event in transformed_results
    ):
...
def _gateway_label(event: Event) -> str:
    ...
    detail = event.payload.get("kind") or event.payload.get("tool") or event.kind
...
def _gateway_preview(event: Event) -> str:
    return _preview(
        event.payload.get("text") or event.payload.get("line") or event.payload
    )
...
def _agent_status(event: dict[str, Any], phase: str) -> EvidenceStatus:
    if phase == "tool_result":
...
def _gateway_status(event: Event) -> EvidenceStatus:
