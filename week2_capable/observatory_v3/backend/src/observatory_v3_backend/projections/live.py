...
def project_live(
    ...
    *,
    ...
    selected_at = (
        gateway_prefix[-1].at if through is not None and gateway_prefix else None
    )
    agent_prefix = [
        event
    ...
    vitals_event = next(
        (
            ...
            if event.kind == "observation" and event.payload.get("kind") == "vitals"
        ),
        None,
    )
    ...
    if atlas_contexts:
        world = world.model_copy(
            update={
                "nodes": tuple(
                    node.model_copy(update={"atlas": atlas_contexts.get(node.place)})
    ...
    friction = _friction(
        ...
        [event for event in agent_prefix if event.get("phase") == "iteration"],
        world,
    )
    ...
    return LiveJourneySnapshot(
        ...
        tools=tuple(
            tool
            for tool in (prompt.get("tools", ()) if prompt is not None else ())
...
def _operator_messages(
    ...
    *,
    ...
    for message in messages:
        ...
        applied_in_prefix = isinstance(applied_iteration, int) and (
            selected_at is None
        ...
        projected.append(
            LiveOperatorMessage(
                action=("revise" if message.get("action") == "revise" else "guide"),
                instruction=instruction,
                sent_at=sent_at,
                applied_iteration=(applied_iteration if applied_in_prefix else None),
            )
        )
...
def _friction(
    ...
    iterations_since_new_place = (
        max(0, iterations - first_observations[-1][0]) if first_observations else None
    )

    command_events = [
        event
        for event in events
        if event.kind == "command" and (_text(event.payload.get("line")) or "").strip()
    ]
    ...
    if positions:
        ...
        current_room_commands = [
            event for event in command_events if event.seq > room_entry_sequence
        ]
    counts = Counter(
    ...
    elif iterations >= 10 and distinct_places <= max(1, iterations // 10):
        kind = "progress_stall"
...
def _atlas_contexts(
    ...
    for event in events:
        ...
        if event.kind == "command" and event.trace_id is not None:
            direction = _movement_direction(_text(event.payload.get("line")) or "")
            if direction is not None:
                commands[event.trace_id] = (direction, event.seq)
            continue
        if event.kind != "position":
            continue
        command = commands.get(event.trace_id) if event.trace_id is not None else None
        ...
        if target is None or title is None or not _same_title(title, target.room.title):
            return None, contexts
        location = target
...
def _graph_atlas_contexts(
    ...
    edges_by_node: dict[str, list[WorldEdge]] = {node_id: [] for node_id in nodes}
...
def _same_exits(observed: tuple[str, ...], expected: tuple[str, ...]) -> bool:
    normalized = {
        _movement_direction(direction) or direction.casefold() for direction in observed
    }
    ...
    if current is not None:
        ...
        if routes:
            ...
            beacon_evidence = tuple(
                f"objective beacon seq {sequence}" for sequence in beacon.evidence
            )
            return LiveSuggestedAction(
                kind="route",
                label=f"Head to {beacon.label}",
                instruction=(
                    f"Follow the learned route toward {beacon.label}: {directions}."
                ),
                reason=(
...
def _route_to(
    ...
    queue: deque[tuple[str, tuple[WorldEdge, ...]]] = deque([(source, ())])
    visited = {source}
    while queue:
    ...
    for event in gateway_events:
        ...
        if observation_kind == "combat":
            ...
            if switched:
                ...
                assert combat_command is not None
                episode["active"] = False
            ...
            if episode is None or not episode["active"]:
                command_sequence = (
                    combat_command[2] if combat_command is not None else None
                )
                evidence = (
                ...
                episode = {
                    ...
                    "outcome": None,
                    "command_trace": (
                        event.trace_id if combat_command is not None else None
                    ),
                    "lines": [],
                    "evidence": evidence,
                }
...
def _turn_at(
    ...
    count = sum(
        1 for event in response_events if 0 < _stamp(event.get("at")) <= observed_at
    )
...
def derive_player_status(
    ...
    for event in events:
        if event.kind != "observation" or event.payload.get("kind") != "player_state":
            continue
        ...
        for name in ("hit", "mana", "move"):
            ...
            if isinstance(value, int):
                fields[name] = LiveObservedValue(
                    ...
                    confidence=(
                        _text(vitals_event.payload.get("confidence")) or "unknown"
                    ),
...
def _current_turn_cost(events: list[dict[str, Any]]) -> float:
    start = max(
        (index for index, event in enumerate(events) if event.get("phase") == "turn"),
        default=0,
    )
    return sum(
...
def _room_economics(
    ...
    positions = [event for event in gateway_events if event.kind == "position"]
    ...
    for response_number, response in enumerate(responses, start=1):
        ...
        latest_position = next(
            (event for event in reversed(positions) if stamp > 0 and event.at <= stamp),
            None,
        )
        place = (
        ...
        node_id = (
            ...
            if isinstance(place, int) and confidence not in {"ambiguous", "unknown"}
            else None
        )
...
def _rooms(
    ...
    rooms: list[LiveRoom] = []
    current_place = positions[-1].payload.get("place") if positions else None
    for place, events in grouped.items():
        latest = events[-1]
        source = _source_observation(latest, observation_by_seq, observations)
        raw_exits = source.payload.get("exits") if source is not None else ()
        exits = raw_exits if isinstance(raw_exits, (list, tuple)) else ()
        rooms.append(
            LiveRoom(
                id=f"place-{place}",
                place=place,
                title=_text(latest.payload.get("title")) or f"Place {place}",
                exits=tuple(value for value in exits if isinstance(value, str)),
...
def _quiet_cohorts(
    ...
    combat = [item for item in timeline if "combat" in item.kind.lower()]
    if combat:
    ...
    for item in timeline:
        ...
        tagged.append(
            item.model_copy(update={"quiet_cohort": f"quiet-{cohort_number}"})
        )
    return tuple(tagged)
                ...
                for block in reversed(content):
                    if isinstance(block, dict) and block.get("type") == "text":
                        ...
                        if text and not text.startswith(
                            "Authenticated operator guidance for the active objective:"
                        ):
                            return text
    return None
    ...
    *,
    ...
    for event in events:
        if event.get("phase") == "session_start":
            ...
            current = authored
            continue
        if event.get("phase") != "operator_control" or event.get("action") != "revise":
            continue
...
def _agent_label(event: dict[str, Any]) -> str:
    ...
    if phase == "operator_control":
        ...
        return (
            f"Operator {action}: {instruction}" if instruction else f"Operator {action}"
        )
