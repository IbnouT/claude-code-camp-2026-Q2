...
def replay_parser(database: Path, mode: Mode) -> ParserCounterfactual:
    ...
    try:
        recorded = connection.execute(
            "SELECT payload FROM events WHERE kind = 'parse_metric' ORDER BY seq"
        ).fetchall()
    ...
    for (encoded,) in recorded:
        ...
        wire_ref = dict(payload.get("wire_ref") or {})
        digest = str(wire_ref.get("digest") or "")
        body = blobs.get(digest)
        if body is not None:
            frames.append((int(wire_ref.get("first_seq") or 0), bytes(body)))
    ...
    recorded_miss = (
        (recorded_lines - recorded_typed) / recorded_lines if recorded_lines else 0
    )
