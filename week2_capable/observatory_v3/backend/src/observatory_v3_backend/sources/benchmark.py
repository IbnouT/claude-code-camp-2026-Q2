...
class BenchmarkSource:
    ...
    def _investigate(
        ...
        agent_path = ledger.parent / "attempts" / summary.attempt / "agent.jsonl"
        ...
        return Investigation(
            ...
            world=project_world(
                ledger.parent / "attempts" / summary.attempt / "gateway.db"
            ),
        )
...
def stable_run_id(ledger_name: str, attempt: str) -> str:
    ...
    return hashlib.sha256(f"{ledger_name}:{attempt}".encode()).hexdigest()[:16]
...
def _diagnostics(
    ...
    if not summary.success and summary.stop_reason == "completed":
        ...
        diagnostics.append(
            DiagnosticRecord(
                ...
                at=final_seq,
                evidence=tuple(
                    item for item in (response_id, outcome_id) if item in citations
                ),
            )
        )
    ...
    repeated = [
        (room, count)
        for room, count in Counter(room_visits).most_common(3)
        if count >= 5
    ]
    if repeated:
        ...
        citations[loop_id] = EvidenceCitation(
            ...
            excerpt=", ".join(f"{room} x{count}" for room, count in repeated),
        )
        diagnostics.append(
            DiagnosticRecord(
                id="confusion-loop",
                ...
                detail=", ".join(
                    f"{room} was seen {count} times" for room, count in repeated
                ),
                mechanism=(
    ...
    if stall >= 10:
        ...
        citations[stall_id] = EvidenceCitation(
            ...
            excerpt=(
                f"{stall} room observations elapsed without discovering a new title."
            ),
        )
        diagnostics.append(
            DiagnosticRecord(
...
def _lens(
    ...
    result_citation = f"agent:{rows.index(final_result) + 1}" if final_result else None
    return EvidenceLens(
        wire=EvidenceForm(
        ...
        parsed=EvidenceForm(
            ...
            citations=tuple(
                item for item in ("gateway:final-position",) if item in citations
            ),
        ),
