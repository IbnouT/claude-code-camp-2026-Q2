    ...
    if selected_id is not None:
        ...
        records = [
            ...
            if (
                ...
                or (record.sequence == selected.sequence and record.id == selected.id)
            )
        ]
...
def diagnose_stop(
    ...
    investigation = (
        benchmark.investigation(request.scope.run_id) if request.scope.run_id else None
    )
    steps = (
        QueryStep(
            ...
            source="agent",
            detail=(
                "Locate the final model claim available at the selected replay moment."
            ),
        ),
        QueryStep(
    ...
    finding = next(
        (item for item in investigation.diagnostics if item.kind == "false_completion"),
        None,
    )
    if finding is None:
        return AskResponse(
    ...
    cited_ids = {citation for claim in claims for citation in claim.citations}
    citations = tuple(item for item in investigation.citations if item.id in cited_ids)
    return AskResponse(
        tier="deterministic",
        question=request.question,
        scope_record_id=request.scope.selected_record_id,
...
def _before_final_response(
    ...
    final = next(
        (row for row in reversed(bundle.agent_rows) if row.get("phase") == "response"),
        None,
    )
