...
def samples(
    ...
    if query.order == "cost_desc":
        comparison_selected.sort(key=lambda sample: sample.cost_usd, reverse=True)
    citations = tuple(
        EvidenceCitation(
        ...
        for sample in comparison_selected[: query.limit]
    )
    return AskResponse(
        tier="deterministic",
        question=request.question,
        plan=(step,),
        answer=(
            f"{len(comparison_selected)} matching samples belong to comparison "
