...
def add_session(
    ...
    *,
    ...
    operator_socket = Path(tempfile.gettempdir()) / f"boukensha-{digest}-operator.sock"
    ...
    database.execute(
        ...
        INSERT INTO sessions (
            ...
            updated_at, ended_at, exit_code, stop_mode, capture_status, legacy
        ) VALUES (
            ?, ?, ?, ?, ?, NULL, NULL, ?, ?, ?, ?, NULL, ?, ?, ?, 0,
            NULL, ?, 0
        )
...
async def test_catalog_discovers_all_players_and_session_states(tmp_path: Path):
    ...
    sources = {source["id"]: source for source in capabilities.json()["sources"]}
...
async def test_runtime_session_investigation_opens_any_launcher_run(
    ...
    assert {(record["source"], record["kind"]) for record in payload["records"]} >= {
        ("agent", "session_start"),
        ("agent", "model_request"),
        ("agent", "response"),
...
async def test_objective_revisions_and_nudges_remain_distinct_everywhere(
    ...
    assert controls["guidance"]["preview"] == ("Return through the western gate")
...
async def test_runtime_investigation_exposes_complete_model_exchange(
    ...
    root = runtime_root(tmp_path)
    agent_log = root / "profiles" / "beta" / "sessions" / "session-beta" / "agent.jsonl"
    identity = {
        "player_id": "beta",
        "agent_id": "agent-beta",
    ...
    assert (
        by_kind["provider_response"]["fields"]["response"]["content"][0]["text"]
        == "I will explore as Beta."
    )
...
async def test_wire_evidence_drills_to_integrity_checked_bytes(tmp_path: Path):
    ...
    assert parsed_record["fields"]["text"] == ("Available pets are:\n  300 - the puppy")
    decoded_record = next(
        record
        for record in records
        if record["source"] == "gateway" and record["kind"] == "wire_text"
    )
    parser_input_record = next(
        record
        for record in records
        if record["source"] == "gateway" and record["kind"] == "parser_input"
    )
...
def test_empty_gateway_database_is_a_valid_zero_event_capture(
    ...
    root = runtime_root(tmp_path)
    journal = root / "profiles" / "beta" / "sessions" / "session-beta" / "gateway.db"
    journal.unlink()
    journal.touch()
...
async def test_live_snapshot_exposes_observed_status_economics_and_frontier(
    ...
    assert {
        (item["source"], item["direction"]) for item in snapshot["world"]["frontier"]
    } == {
        ("place:3042", "east"),
        ("place:3042", "west"),
    }
...
async def test_operator_guidance_and_revised_goal_are_visible_evidence(
    ...
    root = runtime_root(tmp_path)
    log = root / "profiles" / "alpha" / "sessions" / "session-alpha" / "agent.jsonl"
    identity = {
        "player_id": "alpha",
        "agent_id": "agent-alpha",
        "session_id": "session-alpha",
            ...
            json.dumps(
                {
                    ...
                    "messages": [
                        {
                            "role": "user",
                            "content": [{"type": "text", "text": "Explore as Alpha"}],
                        },
                        {
                            "role": "user",
                            "content": [
...
async def test_control_targets_only_the_selected_live_agent(tmp_path: Path):
    root = runtime_root(tmp_path)
    digest = hashlib.sha256(b"session-alpha").hexdigest()[:20]
    socket_path = Path(tempfile.gettempdir()) / f"boukensha-{digest}-operator.sock"
    received: list[dict] = []
...
def test_gateway_quarantine_dominates_operator_pause(tmp_path: Path):
    ...
    assert ControlRepository._effective_state(tmp_path) == "quarantined"
...
async def test_zone_follows_verified_reset_and_directional_atlas_chain(
    ...
    target_node = next(
        node for node in correlated["world"]["nodes"] if node["place"] == 2
    )
