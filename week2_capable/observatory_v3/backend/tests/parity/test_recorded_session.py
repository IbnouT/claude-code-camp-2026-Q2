...
from observatory_v3_backend.projections.session import (
    _objective,
    project_recorded_session,
    project_recorded_session_prefix,
)
from observatory_v3_backend.settings import Settings
from observatory_v3_backend.sources.recorded_session import RecordedSessionSource
...
def _fixture(tmp_path):
    ...
    connection.executemany(
        ...
        [(*row[:6], json.dumps(row[6])) for row in gateway_rows],
    )
    connection.commit()
    connection.close()
    return root
...
async def test_recorded_session_routes_name_the_experiment_relationship(
    ...
    assert detail.json()["correlation"].startswith("This recorded session is linked")
