...
from observatory_v3_backend.contracts import IncidentCapsule, QueryScope
from observatory_v3_backend.execution import ExperimentExecutor
from observatory_v3_backend.incidents import canonical_payload
from observatory_v3_backend.projections.parser_replay import replay_parser
from observatory_v3_backend.projections.world import project_world, project_world_events
from observatory_v3_backend.queries import plan_query
from observatory_v3_backend.queries.model import ModelTranslator
from observatory_v3_backend.redaction import redact_question
...
def test_copilot_query_corpus_routes_only_supported_operations():
    fixture = Path(__file__).parent / "fixtures" / "copilot_queries.json"
...
def test_model_boundary_redacts_secret_shaped_question_text():
    value = redact_question(
        "Why stopped? password=hunter2 token:abc123 0123456789abcdef0123456789abcdef"
    )
...
""",
    ...
    assert "environment-only-secret" not in (config / "settings.yaml").read_text(
        encoding="utf-8"
    )
...
async def test_world_atlas_uses_zone_lod_without_collapsing_titles(tmp_path):
    ...
    async with httpx.AsyncClient(
        ...
        zone = (await client.get("/api/world/atlas?level=zone&zone=7")).json()
...
async def test_j2_false_completion_links_claim_to_verified_outcome(tmp_path):
    ...
    async with httpx.AsyncClient(
        ...
        runs = (await client.get("/api/runs")).json()["runs"]
        response = await client.get(f"/api/runs/{runs[0]['id']}/investigation")
        ...
        translated = await client.post(
            ...
            json={
                "question": (
                    "I need an autopsy of the final decision token=private-value"
                ),
                "scope": {
                    "space": "sessions",
                    "run_id": runs[0]["id"],
...
async def test_incident_capsule_is_sanitized_integrity_sealed_and_portable(
    ...
    async with httpx.AsyncClient(
        ...
        runs = (await client.get("/api/runs")).json()["runs"]
        run_id = runs[0]["id"]
        knowledge = await client.get(f"/api/runs/{run_id}/knowledge-projection")
                ...
                "annotations": [
                    {
                        ...
                        "bookmark": True,
                        "text": (
                            "Check /Users/reviewer/private/run.json token=private-value"
                        ),
    ...
    assert (
        capsule.digest == hashlib.sha256(canonical_payload(capsule.payload)).hexdigest()
    )
...
def test_world_projection_keeps_duplicate_titles_as_distinct_candidates(
    ...
    connection.executemany(
        ...
        [(seq, kind, trace, json.dumps(payload)) for seq, kind, trace, payload in rows],
    )
    connection.commit()
    connection.close()
