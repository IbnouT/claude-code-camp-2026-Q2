...
const edgeOrder: FocusContinuationEdge[] = [
  "top",
  "right",
  "bottom",
  "left",
...
function projectFootprintToEdge(
  ...
  if (footprint.x + footprint.width > viewport.x + viewport.width) {
    candidates.push({
      edge: "right",
      overshoot: footprint.x + footprint.width
  ...
  if (footprint.y + footprint.height > viewport.y + viewport.height) {
    candidates.push({
      edge: "bottom",
      overshoot: footprint.y + footprint.height
  ...
  candidates.sort((left, right) => {
    return left.overshoot - right.overshoot
